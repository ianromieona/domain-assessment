  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      h.general.alert('success', {
        message: `Note successfully ${selectedNote ? 'updated' : 'created'}`,
      });
      onSuccess(data);
    } catch (error) {
      ...
      <form onSubmit={handleSubmit} className="p-2">
        <div className="mb-4">
          ...
          <input
            ...
            name="title"
            id="title"
            {...(selectedNote && { defaultValue: selectedNote.title })}
        ...
        <div className="flex justify-between">
          <button
            type="button"
            className="bg-gray-500 text-white px-3 py-2 text-xs rounded-md "
            onClick={(e) => {
              e.preventDefault();
              onCancel();
            }}
          ...
          <button
            type="submit"
            className="bg-blue-500 text-white px-3 py-2 text-xs rounded-md "
            disabled={loading}
          >
